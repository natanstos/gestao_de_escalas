...
import { AlertTriangle, Building2, CalendarDays, Check, ChevronRight, ClipboardList, Download, FileText, History, LayoutDashboard, Lock, LockOpen, Menu, MessageCircle, Plus, RefreshCw, Save, Scale, Search, Settings, Share2, ShieldCheck, Sparkles, UserPlus, Users, X } from "lucide-react";
...
type ApiAssignment = { id: string; eventId: string; stationId: string; locked: boolean; event: { id: string; title: string; startsAt: string; eventType: { code: string; color: string } }; station: { id: string; name: string; sortOrder: number }; workerId: string; worker: { displayName: string }; status: string };
type ApiEvent = { id: string; title: string; startsAt: string; visibleInSchedule: boolean; eventType: { code: string; color: string }; requirements: Array<{ quantity: number; station: { id: string; name: string; sortOrder: number } }> };
...
type StationItem = { id: string; name: string; defaultQuantity: number; active: boolean; sortOrder: number };
type EventTypeItem = { id: string; name: string; code: string; weekday?: number | null; defaultTime?: string | null; stations: Array<{ stationId: string; quantity: number; enabled: boolean; station: StationItem }> };
  ...
  return events.map(event => {
    ...
    return {
      ...
      assignments: event.requirements.map(requirement => {
        ...
        return { station: requirement.station.name, names: group.map(item => item.worker.displayName), slots: group.map(item => ({ id: item.id, workerId: item.workerId, name: item.worker.displayName, locked: item.locked })), status: (group.length < requirement.quantity ? "open" : group.some(item => item.status === "PENDING") ? "pending" : "confirmed") as "open" | "pending" | "confirmed" };
      })
    };
...
export default function App() {
  ...
  useEffect(() => {
    Promise.all([fetch("/api/dashboard"), fetch("/api/workers"), fetch("/api/stations"), fetch("/api/event-types")]).then(async ([dashboardResponse, workersResponse, stationsResponse, eventTypesResponse]) => {
      if (!dashboardResponse.ok || !workersResponse.ok || !stationsResponse.ok || !eventTypesResponse.ok) throw new Error("Falha ao carregar dados");
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {view === "dashboard" && <Dashboard services={scheduleServices.filter(service => service.visible !== false)} onNavigate={navigate} onShare={() => setShareOpen(true)}/>}
      ...
      {view === "settings" && <SettingsPage settings={churchSettings} setSettings={setChurchSettings} scheduleId={scheduleId} services={scheduleServices} setServices={setScheduleServices} stations={stations} setStations={setStations} eventTypes={eventTypes} setEventTypes={setEventTypes} announce={announce}/>}
    </main>
    ...
    {shareOpen && <ShareModal services={scheduleServices.filter(service => service.visible !== false)} shareRef={shareRef} onClose={() => setShareOpen(false)} onShare={shareImage} announce={announce}/>}
...
function SchedulePage({ scheduleId, services, setServices, workers, onShare, announce }: { scheduleId: string | null; services: Service[]; setServices: React.Dispatch<React.SetStateAction<Service[]>>; workers: WorkerItem[]; onShare: () => void; announce: (m: string) => void }) {
  ...
  const [validation, setValidation] = useState<{ valid: boolean; summary: { errors: number; warnings: number; locked: number }; issues: Array<{ severity: string; title: string; detail: string }> } | null>(null);
  const [revisions, setRevisions] = useState<Array<{ id: string; version: number; reason: string; createdAt: string }>>([]);
  ...
  const openDaySelector = () => { setSelectedEventIds(services.filter(service => service.visible !== false).map(service => service.id)); setDaySelectorOpen(true); };
  const saveVisibleDays = async () => {
    if (!scheduleId) return;
    try {
      const response = await fetch(`/api/schedules/${scheduleId}/visible-events`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ eventIds: selectedEventIds.map(String) }) });
      if (!response.ok) throw new Error();
      setServices(current => current.map(service => ({ ...service, visible: selectedEventIds.includes(service.id) })));
      setDaySelectorOpen(false); announce("Dias exibidos na escala foram atualizados. Gere novamente para redistribuir os obreiros.");
    } catch { announce("Não foi possível salvar os dias da escala."); }
  };
  ...
  const reviewBeforeGenerate = async () => {
    ...
    try { const response = await fetch(`/api/schedules/${scheduleId}/validate`); if (!response.ok) throw new Error(); setValidation(await response.json()); setConfirmGenerate(true); }
    catch { announce("Não foi possível validar a escala."); }
  };
  const openHistory = async () => {
    if (!scheduleId) return;
    try { const response = await fetch(`/api/schedules/${scheduleId}/revisions`); if (!response.ok) throw new Error(); setRevisions(await response.json()); setHistoryOpen(true); }
    catch { announce("Não foi possível carregar as versões."); }
  };
  const restoreRevision = async (revisionId: string) => {
    if (!scheduleId) return;
    try { const response = await fetch(`/api/schedules/${scheduleId}/revisions/${revisionId}/restore`, { method: "POST" }); if (!response.ok) throw new Error(); const restored = await response.json(); const dashboard = await (await fetch("/api/dashboard")).json(); setServices(mapSchedule(dashboard.events, restored.assignments)); setHistoryOpen(false); announce("Versão restaurada. O estado anterior também foi guardado."); }
    catch { announce("Não foi possível restaurar esta versão."); }
  };
  const toggleLock = async (serviceId: string | number, assignmentId: string, locked: boolean) => {
    try { const response = await fetch(`/api/assignments/${assignmentId}/lock`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ locked }) }); if (!response.ok) throw new Error(); setServices(current => current.map(service => service.id !== serviceId ? service : { ...service, assignments: service.assignments.map(item => ({ ...item, slots: item.slots?.map(slot => slot.id === assignmentId ? { ...slot, locked } : slot) })) })); announce(locked ? "Alocação protegida." : "Alocação liberada para a próxima geração."); }
    catch { announce("Não foi possível alterar a proteção."); }
  };
  const saveAssignment = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setServices(current => current.map(service => service.id !== editingAssignment.serviceId ? service : { ...service, assignments: service.assignments.map(assignment => {
        ...
        const slots = assignment.slots.map(slot => slot.id === editingAssignment.assignmentId ? { ...slot, workerId: body.workerId, name: body.worker.displayName, locked: true } : slot);
  ...
  return <div className="page"><div className="page-toolbar"><div className="month-switch"><button onClick={() => announce("Julho de 2026 não possui escala neste MVP.")}>‹</button><strong>Agosto de 2026</strong><button onClick={() => announce("Setembro de 2026 ainda não foi criado.")}>›</button></div><div><button className="ghost" onClick={openHistory}><History size={18}/> Versões</button><button className="ghost" onClick={openDaySelector}><CalendarDays size={18}/> Selecionar dias</button><button className="ghost" onClick={reviewBeforeGenerate}><Sparkles size={18}/> Gerar novamente</button><button className="primary" onClick={onShare}><MessageCircle size={18}/> Compartilhar</button></div></div><div className="month-summary"><CalendarDays/><strong>{visibleServices.length} cultos selecionados</strong><span>Alterações manuais protegidas permanecem intactas ao gerar novamente.</span></div><div className="schedule-grid">{visibleServices.map(service => <article className="service-card" key={service.id}><div className="service-head"><div className={`large-date ${service.color}`}><strong>{service.date.split(" ")[0]}</strong><span>{service.date.split(" ")[1]}</span></div><div><span className="eyebrow">{service.weekday} · {service.time}</span><h3>{service.title}</h3></div><button className="icon-button edit-service" aria-label={`Editar ${service.title}`} onClick={() => setEditing(service)}>•••</button></div><div className="assignment-list">{service.assignments.map(a => <div key={a.station} className={a.status === "open" ? "assignment open-slot" : "assignment"}><span>{a.station}</span><div className="assignment-workers">{a.slots?.map(slot => <div className={slot.locked ? "assignment-chip locked" : "assignment-chip"} key={slot.id}><button onClick={() => setEditingAssignment({ serviceId: service.id, assignmentId: slot.id, workerId: slot.workerId, station: a.station, currentName: slot.name })}>{slot.name}<span>Editar</span></button><button className="slot-lock" aria-label={slot.locked ? `Liberar ${slot.name}` : `Proteger ${slot.name}`} title={slot.locked ? "Protegido na geração" : "Proteger na geração"} onClick={() => toggleLock(service.id, slot.id, !slot.locked)}>{slot.locked ? <Lock size={14}/> : <LockOpen size={14}/>}</button></div>)}{!a.slots?.length && <strong>+ Preencher vaga</strong>}</div>{a.status === "confirmed" ? <Check className="ok"/> : a.status === "pending" ? <span className="pending-dot"/> : <Plus/>}</div>)}</div></article>)}</div>
    {confirmGenerate && <div className="modal-backdrop"><section className="confirm-modal validation-modal"><div className="stat-icon purple"><ShieldCheck/></div><h2>Revisão antes de gerar</h2><p>{validation?.summary.locked ?? 0} alocação(ões) protegida(s) serão preservadas. A versão atual será guardada para restauração.</p>{validation && <div className="validation-summary"><span className={validation.summary.errors ? "has-errors" : "is-valid"}><strong>{validation.summary.errors}</strong> conflitos</span><span><strong>{validation.summary.warnings}</strong> alertas</span><span><strong>{validation.summary.locked}</strong> protegidas</span></div>}<div className="validation-issues">{validation?.issues.filter(issue => issue.severity !== "info").slice(0,6).map((issue,index) => <article key={`${issue.title}-${index}`} className={issue.severity}><AlertTriangle size={17}/><div><strong>{issue.title}</strong><span>{issue.detail}</span></div></article>)}{validation?.issues.filter(issue => issue.severity !== "info").length === 0 && <article className="success"><Check size={17}/><div><strong>Nenhum conflito encontrado</strong><span>A escala está pronta para uma nova distribuição.</span></div></article>}</div><div className="modal-actions"><button className="ghost" onClick={() => setConfirmGenerate(false)}>Cancelar</button><button className="primary" onClick={regenerate}>Gerar e criar nova versão</button></div></section></div>}
    {historyOpen && <div className="modal-backdrop"><section className="form-modal history-modal"><div className="modal-head"><div><span className="eyebrow">HISTÓRICO</span><h2>Versões da escala</h2></div><button className="icon-button" onClick={() => setHistoryOpen(false)}><X/></button></div><p className="form-help">Cada geração guarda a distribuição anterior. Restaurar também cria um ponto de retorno.</p><div className="revision-list">{revisions.map(revision => <article key={revision.id}><div><strong>Versão {revision.version}</strong><span>{revision.reason} · {new Intl.DateTimeFormat("pt-BR", { dateStyle: "short", timeStyle: "short" }).format(new Date(revision.createdAt))}</span></div><button className="secondary" onClick={() => restoreRevision(revision.id)}><RefreshCw size={15}/> Restaurar</button></article>)}{revisions.length === 0 && <div className="empty-state">A primeira versão será criada ao gerar novamente.</div>}</div></section></div>}
    ...
    {daySelectorOpen && <div className="modal-backdrop"><section className="form-modal day-selector-modal"><div className="modal-head"><div><span className="eyebrow">DIAS DA ESCALA</span><h2>Quais cultos devem aparecer?</h2></div><button className="icon-button" onClick={() => setDaySelectorOpen(false)}><X/></button></div><p className="form-help">Desmarque os dias anteriores à publicação ou qualquer culto que não deve fazer parte desta escala.</p><div className="day-selector-actions"><button className="text-button" onClick={() => setSelectedEventIds(services.map(service => service.id))}>Marcar todos</button><button className="text-button" onClick={() => { const today = new Date(); setSelectedEventIds(services.filter(service => !service.isoDate || new Date(service.isoDate) >= new Date(today.getFullYear(), today.getMonth(), today.getDate())).map(service => service.id)); }}>A partir de hoje</button><button className="text-button muted-text" onClick={() => setSelectedEventIds([])}>Limpar</button></div><div className="day-options">{services.map(service => <label className={selectedEventIds.includes(service.id) ? "selected" : ""} key={service.id}><input type="checkbox" checked={selectedEventIds.includes(service.id)} onChange={event => setSelectedEventIds(current => event.target.checked ? [...current, service.id] : current.filter(id => id !== service.id))}/><div className={`mini-date ${service.color}`}><strong>{service.date.split(" ")[0]}</strong><span>{service.weekday}</span></div><div><strong>{service.title}</strong><span>{service.time}</span></div></label>)}</div><div className="modal-actions"><button className="ghost" onClick={() => setDaySelectorOpen(false)}>Cancelar</button><button className="primary" onClick={saveVisibleDays}><Save size={17}/> Salvar dias selecionados</button></div></section></div>}
  </div>;
}
...
function SettingsPage({ settings, setSettings, scheduleId, services, setServices, stations, setStations, eventTypes, setEventTypes, announce }: { settings: { name: string; timezone: string }; setSettings: React.Dispatch<React.SetStateAction<{ name: string; timezone: string }>>; scheduleId: string | null; services: Service[]; setServices: React.Dispatch<React.SetStateAction<Service[]>>; stations: StationItem[]; setStations: React.Dispatch<React.SetStateAction<StationItem[]>>; eventTypes: EventTypeItem[]; setEventTypes: React.Dispatch<React.SetStateAction<EventTypeItem[]>>; announce: (m: string) => void }) {
  const [tab, setTab] = useState("geral");
  const [stationEditor, setStationEditor] = useState<StationItem | "new" | null>(null);
  const supperService = services.find(service => service.color === "purple");
  const refreshEventTypes = async () => { const response = await fetch("/api/event-types"); if (response.ok) setEventTypes(await response.json()); };
  const saveStation = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(editing ? `/api/stations/${editing.id}` : "/api/stations", { method: editing ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: String(form.get("name")), defaultQuantity: Number(form.get("quantity")), active: editing?.active ?? true }) });
      if (!response.ok) throw new Error(); const saved = await response.json();
      setStations(current => editing ? current.map(item => item.id === saved.id ? saved : item) : [...current, saved]);
      await refreshEventTypes(); setStationEditor(null); announce(editing ? "Posição atualizada." : "Posição cadastrada para os cultos.");
    } catch { announce("Não foi possível salvar a posição."); }
  };
  const saveSupperDate = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault(); if (!scheduleId) return;
    const form = new FormData(event.currentTarget); const date = String(form.get("supperDate")); const time = String(form.get("supperTime"));
    try {
      const response = await fetch(`/api/schedules/${scheduleId}/santa-ceia`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ startsAt: `${date}T${time}:00-03:00` }) });
      if (!response.ok) { const body = await response.json(); throw new Error(body.message); }
      const dashboardResponse = await fetch("/api/dashboard"); const dashboard = await dashboardResponse.json();
      setServices(mapSchedule(dashboard.events, dashboard.schedule.assignments));
      announce("Data da Santa Ceia atualizada. Gere novamente a escala para preencher os cultos alterados.");
    } catch (error) { announce(error instanceof Error && error.message ? error.message : "Não foi possível alterar a data da Santa Ceia."); }
  };
  return <div className="page settings-page"><div className="settings-tabs"><button className={tab === "geral" ? "active" : ""} onClick={() => setTab("geral")}>Geral</button><button className={tab === "cultos" ? "active" : ""} onClick={() => setTab("cultos")}>Cultos e posições</button><button className={tab === "compartilhar" ? "active" : ""} onClick={() => setTab("compartilhar")}>Compartilhamento</button></div>
    {tab === "geral" && <form className="settings-card" onSubmit={async event => { event.preventDefault(); const form = new FormData(event.currentTarget); try { const response = await fetch("/api/church", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: String(form.get("name")), timezone: String(form.get("timezone")) }) }); if (!response.ok) throw new Error(); const saved = await response.json(); setSettings({ name: saved.name, timezone: saved.timezone }); announce("Configurações salvas no PostgreSQL."); } catch { announce("Não foi possível salvar as configurações."); } }}><div className="settings-title"><Building2/><div><h3>Dados da congregação</h3><p>Informações exibidas nas escalas e comunicações.</p></div></div><label>Nome da congregação<input name="name" defaultValue={settings.name}/></label><div className="form-grid"><label>Fuso horário<select name="timezone" defaultValue={settings.timezone}><option value="America/Sao_Paulo">Brasília — America/Sao_Paulo</option></select></label><label>Telefone<input placeholder="Telefone institucional"/></label></div><button className="primary save-button"><Save/> Salvar alterações</button></form>}
    {tab === "cultos" && <div className="cult-settings"><form className="settings-card supper-date-card" onSubmit={saveSupperDate}><div className="settings-title"><Sparkles/><div><h3>Data da Santa Ceia</h3><p>Escolha em qual dia do mês o culto especial será realizado.</p></div></div><div className="supper-date-fields"><label>Data<input name="supperDate" type="date" defaultValue={(supperService?.isoDate ?? "2026-08-09").slice(0,10)} required/></label><label>Horário<input name="supperTime" type="time" defaultValue={supperService?.time ?? "18:00"} required/></label><button className="primary"><Save size={17}/> Salvar data</button></div><div className="supper-date-note"><AlertTriangle size={17}/><span>Se a nova data for um domingo, a Santa Ceia substituirá automaticamente o culto normal daquele dia.</span></div></form><section className="settings-card"><div className="settings-title settings-title-action"><div className="settings-title-copy"><Scale/><div><h3>Cadastro de posições</h3><p>Defina os postos disponíveis e a quantidade padrão de obreiros.</p></div></div><button className="primary" onClick={() => setStationEditor("new")}><Plus size={17}/> Nova posição</button></div><div className="station-cards">{stations.filter(station => station.active).map(station => <button key={station.id} onClick={() => setStationEditor(station)}><div><strong>{station.name}</strong><span>{station.defaultQuantity} {station.defaultQuantity === 1 ? "obreiro" : "obreiros"} por padrão</span></div><ChevronRight/></button>)}</div></section><section className="settings-card"><div className="settings-title"><CalendarDays/><div><h3>Posições por tipo de culto</h3><p>Marque os postos utilizados e ajuste a quantidade para cada culto.</p></div></div><div className="event-type-configs">{eventTypes.map(eventType => <EventTypePositions key={eventType.id} eventType={eventType} stations={stations.filter(station => station.active)} onSaved={saved => setEventTypes(current => current.map(item => item.id === saved.id ? saved : item))} announce={announce}/>)}</div></section></div>}
    {tab === "compartilhar" && <form className="settings-card" onSubmit={event => { event.preventDefault(); announce("Preferências de compartilhamento salvas neste navegador."); }}><div className="settings-title"><Share2/><div><h3>Arte para WhatsApp</h3><p>Escolha como a escala será apresentada ao grupo.</p></div></div><label>Divisão da imagem<select defaultValue="mensal"><option value="mensal">Mês completo</option><option value="quinzenal">Por quinzena</option><option value="semanal">Por semana</option></select></label><label className="check-line"><input type="checkbox" defaultChecked/> Exibir número da versão e data de publicação</label><button className="primary save-button"><Save/> Salvar preferências</button></form>}
    {stationEditor && <div className="modal-backdrop"><form className="form-modal" onSubmit={saveStation}><div className="modal-head"><div><span className="eyebrow">POSIÇÃO DO CULTO</span><h2>{stationEditor === "new" ? "Nova posição" : "Editar posição"}</h2></div><button type="button" className="icon-button" onClick={() => setStationEditor(null)}><X/></button></div><label>Nome da posição<input name="name" defaultValue={stationEditor === "new" ? "" : stationEditor.name} placeholder="Ex.: Portaria" required/></label><label>Quantidade padrão de obreiros<input name="quantity" type="number" min="1" max="10" defaultValue={stationEditor === "new" ? 1 : stationEditor.defaultQuantity} required/></label><div className="modal-actions"><button type="button" className="ghost" onClick={() => setStationEditor(null)}>Cancelar</button><button className="primary"><Save size={17}/> Salvar posição</button></div></form></div>}
  </div>;
}

function EventTypePositions({ eventType, stations, onSaved, announce }: { eventType: EventTypeItem; stations: StationItem[]; onSaved: (eventType: EventTypeItem) => void; announce: (message: string) => void }) {
  const initial = () => stations.map(station => { const configured = eventType.stations.find(item => item.stationId === station.id); return { stationId: station.id, name: station.name, enabled: configured?.enabled ?? false, quantity: configured?.quantity ?? station.defaultQuantity }; });
  const [positions, setPositions] = useState(initial);
  useEffect(() => setPositions(initial()), [eventType, stations]);
  const save = async () => { try { const response = await fetch(`/api/event-types/${eventType.id}/stations`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ positions }) }); if (!response.ok) throw new Error(); onSaved(await response.json()); announce(`Posições de ${eventType.name} salvas. Gere novamente a escala para redistribuir.`); } catch { announce("Não foi possível salvar as posições do culto."); } };
  return <article className={eventType.code === "SANTA_CEIA" ? "event-type-config supper" : "event-type-config"}><div className="event-type-head"><div><strong>{eventType.name}</strong><span>{eventType.defaultTime ?? "Horário variável"}</span></div><div><button className="text-button" onClick={() => setPositions(current => current.map(item => ({ ...item, enabled: true })))}>Marcar todas</button><button className="text-button muted-text" onClick={() => setPositions(current => current.map(item => ({ ...item, enabled: false })))}>Limpar</button></div></div><div className="position-options">{positions.map(position => <div className={position.enabled ? "position-option selected" : "position-option"} key={position.stationId}><label><input type="checkbox" checked={position.enabled} onChange={event => setPositions(current => current.map(item => item.stationId === position.stationId ? { ...item, enabled: event.target.checked } : item))}/><span>{position.name}</span></label><input aria-label={`Quantidade em ${position.name}`} type="number" min="1" max="10" disabled={!position.enabled} value={position.quantity} onChange={event => setPositions(current => current.map(item => item.stationId === position.stationId ? { ...item, quantity: Math.max(1, Number(event.target.value)) } : item))}/></div>)}</div><button className="secondary save-type-positions" onClick={save}><Save size={15}/> Salvar posições deste culto</button></article>;
}
...
function ShareModal({ services, shareRef, onClose, onShare, announce }: { services: Service[]; shareRef: React.RefObject<HTMLDivElement | null>; onClose: () => void; onShare: () => void; announce: (m: string) => void }) {
  const [shareFormat, setShareFormat] = useState<"month" | "week" | "worker">("month");
  const [selectedWeek, setSelectedWeek] = useState(0);
  const workerNames = [...new Set(services.flatMap(service => service.assignments.flatMap(assignment => assignment.names)))].sort((a,b) => a.localeCompare(b));
  ...
  const displayedWeeks = shareFormat === "week" ? calendarWeeks.filter((_, index) => index === selectedWeek) : calendarWeeks;
  const displayedServices = shareFormat === "worker" ? services.filter(service => service.assignments.some(assignment => assignment.names.includes(selectedWorker))) : displayedWeeks.flatMap(week => week.services);
  const posterWeeks = shareFormat === "worker" ? [{ week: 0, start: 1, end: daysInMonth, services: displayedServices }] : displayedWeeks;
  ...
  return <div className="modal-backdrop"><section className="share-modal calendar-share-modal"><div className="modal-head"><div><span className="eyebrow">PRÉVIA PARA WHATSAPP</span><h2>{shareFormat === "month" ? "Calendário mensal" : shareFormat === "week" ? "Escala semanal" : `Escala de ${selectedWorker}`}</h2></div><button className="icon-button" onClick={onClose} aria-label="Fechar compartilhamento"><X/></button></div><div className="share-format-tabs"><button className={shareFormat === "month" ? "active" : ""} onClick={() => setShareFormat("month")}>Mês completo</button><button className={shareFormat === "week" ? "active" : ""} onClick={() => setShareFormat("week")}>Por semana</button><button className={shareFormat === "worker" ? "active" : ""} onClick={() => setShareFormat("worker")}>Por obreiro</button></div>{shareFormat === "week" && <label className="share-filter">Semana<select value={selectedWeek} onChange={event => setSelectedWeek(Number(event.target.value))}>{calendarWeeks.map((week,index) => <option value={index} key={week.week}>Semana {index + 1} · dias {week.start} a {week.end}</option>)}</select></label>}{shareFormat === "worker" && <label className="share-filter">Obreiro<select value={selectedWorker} onChange={event => setSelectedWorker(event.target.value)}>{workerNames.map(name => <option key={name}>{name}</option>)}</select></label>}<div className="share-layout calendar-share-layout"><div className="poster-wrap calendar-poster-wrap"><div className={`poster calendar-poster editorial-poster format-${shareFormat}`} ref={shareRef}><div className="calendar-poster-top editorial-masthead"><div className="poster-logo"><ClipboardList/></div><div><span>IGREJA DE BRASÍLIA</span><h2>{shareFormat === "worker" ? `Escala de ${selectedWorker}` : "Escala de Obreiros"}</h2></div><strong>{monthLabel.replace(" DE ", " · ")}</strong></div><div className="editorial-subline"><div className="editorial-legend"><span><i className="legend-dot sunday"/>Domingo</span><span><i className="legend-dot tuesday"/>Terça-feira</span><span><i className="legend-dot friday"/>Sexta-feira</span><span><i className="legend-dot supper"/>Santa Ceia</span></div><strong>{displayedServices.length} cultos</strong></div><div className="calendar-weeks">{posterWeeks.map((week, index) => <section className="calendar-week" key={`${week.week}-${shareFormat}`}><div className="calendar-week-head"><strong>{shareFormat === "worker" ? "MINHAS DESIGNAÇÕES" : `SEMANA ${shareFormat === "week" ? selectedWeek + 1 : index + 1}`}</strong><span>{shareFormat === "worker" ? monthLabel : `${String(week.start).padStart(2,"0")} a ${String(week.end).padStart(2,"0")} de agosto`}</span></div><div className="calendar-cult-days">{week.services.map(service => <article className={`calendar-cult-day ${service.color} ${service.eventCode?.toLowerCase() ?? ""} ${service.color === "purple" ? "santa-ceia-featured" : ""}`} key={service.id}>{service.color === "purple" && <div className="supper-ribbon"><Sparkles size={10}/> SANTA CEIA</div>}<div className="calendar-cult-date"><strong>{service.date.split(" ")[0]}</strong><span>{service.weekday === "DOM" ? "DOMINGO" : service.weekday === "TER" ? "TERÇA" : "SEXTA"}</span></div><div className="calendar-service-title"><strong>{service.title}</strong><span>{service.time}</span></div><div className="calendar-assignments">{service.assignments.filter(assignment => shareFormat !== "worker" || assignment.names.includes(selectedWorker)).map(assignment => <div key={assignment.station}><span>{assignment.station}</span><strong className="calendar-worker-names">{shareFormat === "worker" ? <b>{selectedWorker}</b> : assignment.names.length ? assignment.names.map(name => <b key={name}>{name}</b>) : <b>A definir</b>}</strong></div>)}</div></article>)}</div></section>)}</div><div className="poster-footer">Consulte sempre a versão atualizada antes de cada culto · Igreja de Brasília</div></div></div><div className="share-options"><h3>Compartilhar imagem</h3><p>Escolha mês, semana ou cartão individual antes de enviar ao WhatsApp.</p><button className="whatsapp" onClick={onShare}><MessageCircle/> Compartilhar imagem</button><button className="ghost full" onClick={onShare}><Download/> Baixar PNG</button><button className="ghost full" onClick={() => announce("Link público copiado.")}><Share2/> Copiar link público</button><div className="version-note"><Check/><div><strong>{shareFormat === "month" ? "Mês completo" : shareFormat === "week" ? "Semana selecionada" : "Cartão individual"}</strong><span>{displayedServices.length} culto(s) nesta imagem.</span></div></div></div></div></section></div>;
}
