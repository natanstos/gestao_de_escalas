...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {view === "schedule" && <SchedulePage services={scheduleServices} setServices={setScheduleServices} onShare={() => setShareOpen(true)} announce={announce}/>}
...
function SchedulePage({ services, setServices, onShare, announce }: { services: Service[]; setServices: React.Dispatch<React.SetStateAction<Service[]>>; onShare: () => void; announce: (m: string) => void }) {
  const [editing, setEditing] = useState<Service | null>(null);
  const [confirmGenerate, setConfirmGenerate] = useState(false);
  const regenerate = () => {
    const pool = services.flatMap(service => service.assignments.flatMap(item => item.names));
    let cursor = 1;
    setServices(current => current.map(service => ({ ...service, assignments: service.assignments.map(assignment => {
      const quantity = Math.max(assignment.names.length, assignment.station === "Portaria" || assignment.station === "Escadaria" ? 2 : 1);
      const names = Array.from({ length: quantity }, () => pool[cursor++ % pool.length]);
      return { ...assignment, names, status: "pending" as const };
    }) })));
    setConfirmGenerate(false); announce("Nova sugestão gerada. Revise antes de publicar.");
  };
  const saveService = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault(); if (!editing) return;
    const form = new FormData(event.currentTarget);
    setServices(current => current.map(service => service.id === editing.id ? { ...service, title: String(form.get("title")), date: String(form.get("date")).toUpperCase(), time: String(form.get("time")) } : service));
    setEditing(null); announce("Culto atualizado na escala.");
  };
