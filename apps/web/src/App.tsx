...
function ShareModal({ services, shareRef, onClose, onShare, announce }: { services: Service[]; shareRef: React.RefObject<HTMLDivElement | null>; onClose: () => void; onShare: () => void; announce: (m: string) => void }) {
  ...
  const servicesByDay = new Map<number, Service[]>();
  services.forEach(service => { const day = Number(service.date.split(" ")[0]); servicesByDay.set(day, [...(servicesByDay.get(day) ?? []), service]); });
  ...
  return <div className="modal-backdrop"><section className="share-modal calendar-share-modal"><div className="modal-head"><div><span className="eyebrow">PRÉVIA PARA WHATSAPP</span><h2>Calendário mensal da escala</h2></div><button className="icon-button" onClick={onClose} aria-label="Fechar compartilhamento"><X/></button></div><div className="share-layout calendar-share-layout"><div className="poster-wrap calendar-poster-wrap"><div className="poster calendar-poster" ref={shareRef}><div className="calendar-poster-top"><div className="poster-logo"><ClipboardList/></div><div><span>IGREJA DE BRASÍLIA</span><h2>Escala de Obreiros</h2></div><strong>{monthLabel}</strong></div><div className="calendar-weekdays">{["DOM","SEG","TER","QUA","QUI","SEX","SÁB"].map(day => <strong key={day}>{day}</strong>)}</div><div className="calendar-grid">{calendarDays.map((day, index) => <div className={day ? servicesByDay.has(day) ? "calendar-day has-service" : "calendar-day" : "calendar-day outside"} key={`${day ?? "empty"}-${index}`}>{day && <><span className="calendar-day-number">{day}</span>{servicesByDay.get(day)?.map(service => <section className={`calendar-service ${service.color}`} key={service.id}><div className="calendar-service-title"><strong>{service.title}</strong><span>{service.time}</span></div><div className="calendar-assignments">{service.assignments.map(assignment => <div key={assignment.station}><span>{assignment.station}</span><strong>{assignment.names.length ? assignment.names.join(" · ") : "A definir"}</strong></div>)}</div></section>)}</>}</div>)}</div><div className="poster-footer">Versão 1 · Consulte sempre a versão atualizada antes de cada culto</div></div></div><div className="share-options"><h3>Compartilhar calendário</h3><p>A escala está organizada por semanas, facilitando encontrar cada culto e ampliar os nomes no WhatsApp.</p><button className="whatsapp" onClick={onShare}><MessageCircle/> Compartilhar imagem</button><button className="ghost full" onClick={onShare}><Download/> Baixar PNG</button><button className="ghost full" onClick={() => announce("Link público copiado.")}><Share2/> Copiar link público</button><button className="ghost full" onClick={() => announce("Exportação em PDF será adicionada na próxima versão.")}><FileText/> Exportar PDF</button><div className="version-note"><Check/><div><strong>Mês completo</strong><span>Todos os cultos aparecem no respectivo dia.</span></div></div></div></div></section></div>;
}
